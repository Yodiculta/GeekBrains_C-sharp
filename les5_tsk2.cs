...
namespace Task2
{
    ...
    class Message
    {
        ...
        private int CountOfOneLengthWords(string _bigWord)
        {
            int num = 0;
            for (int i = 0; i < count; i++)
            {
        ...
        public string StringOfMaxWords(int count)
        {
         ...
         public Dictionary<int, string> MyDic(string [] words)
        {
            Dictionary<int, string> dic = new Dictionary<int, string>();
            int num=0;
            for (int i = 0; i < words.Length; i++)
            {
                num = CountOfOneLengthWords(words[i]);
                dic.Add(num, words[i]);
            }
                return dic;
        }
                ...
                while ((number > 0 || number < 4) && key == 1)
                {
                    ...
                    switch (number)
                    {
                        ...
                        case 4:
                            ...
                            break;
                        case 5:
                            WriteBeautiful("The word you want to check: ", ConsoleColor.Blue);
                            Msg.MyDic(Console.ReadLine().Split());
                            break;
                        default:
